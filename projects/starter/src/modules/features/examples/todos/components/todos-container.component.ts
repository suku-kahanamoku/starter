...
export class TodosContainerComponent implements OnInit {
  ...
  onAddTodo() {
    ...
    const addedMessage = this.translateService.instant(
      'ng.examples.todos.added.notification',
      { name: this.newTodo }
    );
  ...
  onToggleTodo(todo: Todo) {
    ...
    const newStatus = this.translateService.instant(
      `ng.examples.todos.filter.${todo.done ? 'active' : 'done'}`
    );
    const undo = this.translateService.instant('ng.examples.todos.undo');
    const toggledMessage = this.translateService.instant(
      'ng.examples.todos.toggle.notification',
      { name: todo.name }
    );
  ...
  onRemoveDoneTodos() {
    ...
    const removedMessage = this.translateService.instant(
      'ng.examples.todos.remove.notification'
    );
  ...
  onFilterTodos(filter: TodosFilter) {
    ...
    const filterToMessage = this.translateService.instant(
      'ng.examples.todos.filter.notification'
    );
    const filterMessage = this.translateService.instant(
      `ng.examples.todos.filter.${filter.toLowerCase()}`
    );
